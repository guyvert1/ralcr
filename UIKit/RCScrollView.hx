...
class RCScrollView extends RCView {
	...
	public var contentRect :Rectangle;// You can access directly the contentView, but be carefull
	...
	public var scrollIndicatorInsets :Point;
	...
	function setScrollEnabled (b:Bool) :Bool {
		...
		if (contentView.width > size.width && horizontalSliderSync == null && b) {
			//trace("add horz");
			var scroller_w = Zeta.lineEquationInt (50, size.width-50, contentRect.width, size.width*10, size.width);
		...
		if (contentView.height > size.height && verticalSliderSync == null && b) {
			//trace("add vert");
			var scroller_h = Zeta.lineEquationInt (50, size.height-50, contentRect.height, size.height*10, size.height);
	 ...
	 */
	public function scrollRectToVisible (rect:Rectangle, animated:Bool) :Void {
		
	}
	
	public function zoomToRect (rect:Rectangle, animated:Bool) :Void {
